...
function SavedJobRow({ job, onUnsave, saving }) {
  const to = `/jobs/${job.slug || job._id}`;
  return <tr className="border-t border-[#EDF1ED] text-sm text-[#2E2F2D] hover:bg-[#FBFCFB]"><td className="break-words px-3 py-4 font-semibold sm:px-4">{job.company?.name || "Company unavailable"}</td><td className="break-words px-3 py-4 sm:px-4"><Link to={to} className="font-medium hover:text-[#214740]">{job.title || "Open position"}</Link><p className="mt-1 break-words text-xs text-[#707E79]">{job.department || "Saved role"}</p></td><td className="break-words px-3 py-4 text-xs text-[#707E79] sm:px-4">{job.location || "Location unavailable"}{job.minExperienceYears != null && <span className="mt-1 block">{job.minExperienceYears ? `${job.minExperienceYears}+ yrs experience` : "No minimum experience"}</span>}</td><td className="px-3 py-4 text-right sm:px-4"><div className="flex flex-wrap justify-end gap-2"><Button as={Link} to={to} size="sm" variant="outline">View job</Button><button type="button" onClick={() => onUnsave(job)} disabled={saving} aria-label={`Unsave ${job.title || "job"}`} className="tap-target inline-flex min-h-9 items-center justify-center rounded-lg border border-[#C1EBAD] px-2.5 text-xs font-semibold text-[#214740] hover:bg-[#EAF9E1] disabled:opacity-60"><Bookmark className="mr-1 h-3.5 w-3.5 fill-current" />Remove</button></div></td></tr>;
}