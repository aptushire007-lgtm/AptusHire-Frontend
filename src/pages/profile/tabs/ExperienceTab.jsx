            ...
            {experience.map((item, idx) => (
              <div key={idx} className="relative rounded-2xl border border-slate-200/90 bg-white p-5 shadow-xs dark:border-slate-800 dark:bg-slate-900">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Job Title / Role</label>
                    <input
                      type="text"
                      required
                      value={item.title}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Company / Organization</label>
                    <input
                      type="text"
                      required
                      value={item.company}
                ...
                <div className="mt-3">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Key Responsibilities &amp; Impact</label>
                  <textarea
                    rows={2}
                    value={item.summary}
                ...
                <div className="mt-3">
                  <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">Skills Demonstrated (comma-separated)</label>
                  <input
                    type="text"
                  ...
                  {item.skills?.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {item.skills.map((s) => (
                        <span key={s} className="rounded-full bg-[#EAF9E1] px-2 py-0.5 text-[11px] font-semibold text-[#3B5D52]">
                          {s}
                        </span>
                      ))}
                    </div>
