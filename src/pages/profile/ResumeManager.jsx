...
export default function ResumeManager() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-3xl border border-[#E8E8E4] bg-white p-6 shadow-soft dark:border-[#E8E8E4] dark:bg-white">
            ...
            <label
              ...
              className={`inline-flex items-center gap-2 rounded-2xl px-4 py-2.5 text-xs font-bold !text-white transition-all shadow-xs ${
                activeVersions.length >= 5
                  ...
                  : "cursor-pointer bg-[#FF6B2C] text-white hover:bg-[#FF6B2C]-dark dark:bg-[#FF6B2C] dark:text-white"
              }`}
            >
              {uploading ? (
                <>
        ...
        {loading ? (
          ...
          <div className="mt-8 rounded-2xl border border-dashed border-[#FFCAAF] bg-white p-8 text-center dark:border-[#FFCAAF] dark:bg-white">
      ...
      {reviewModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#F5F5F0]-deep/80 p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-3xl border border-[#E8E8E4] bg-white p-6 shadow-lift">
            <div className="flex items-center gap-2 text-[#FF6B2C]">
              <Sparkles className="h-5 w-5" />
              <h3 className="font-display text-lg font-bold text-[#1A1A1A]">Review Extracted Version</h3>
            </div>
            ...
            <div className="mt-4 space-y-4">
              <div>
                <label className="block text-xs font-bold text-[#1A1A1A]">Version Label</label>
                <input
                  type="text"
                  value={customLabel}
                  onChange={(e) => setCustomLabel(e.target.value)}
                  className="mt-1 w-full rounded-xl border border-[#E8E8E4] bg-[#F5F5F0] px-3 py-2 text-sm text-[#1A1A1A]"
                  placeholder="e.g. Clinical_Psychology_v2.pdf"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-[#1A1A1A]">Target Role Tags</label>
              ...
              <div>
                ...
                <div className="mt-1 max-h-28 overflow-y-auto flex flex-wrap gap-1.5 rounded-xl border border-slate-100 bg-slate-50/50 p-2.5/50">
                  {extractedSkills.map((s) => (
                    <span key={s} className="inline-flex items-center gap-1 rounded-full bg-[#FFE8DC] px-2.5 py-0.5 text-xs font-semibold text-[#FF6B2C]">
                      {s}
...
function ResumeVersionCard({ version, onSetDefault, onArchive, onDelete, onReplace }) {
  ...
  return (
    <div className={`relative flex flex-col justify-between rounded-2xl border p-5 transition-all shadow-xs ${
      version.isDefault
        ? "border-[#FFCAAF] bg-white dark:border-[#FFCAAF] dark:bg-white"
        : "border-[#E8E8E4] bg-white dark:border-[#E8E8E4] dark:bg-white"
    }`}>
      <div>
          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex min-w-0 flex-wrap items-center gap-2">
                ...
                {version.isDefault && (
                  <span className="inline-flex items-center gap-1 rounded-full bg-[#C1EBAD] px-2 py-0.5 text-[11px] font-extrabold text-[#FF6B2C] shadow-2xs">
        ...
        <div className="mt-3.5 flex flex-wrap items-center gap-1.5">
          {skills.slice(0, 5).map((skill) => (
            <span key={skill} className="rounded-full bg-[#FFE8DC] px-2 py-0.5 text-[11px] font-semibold text-[#FF6B2C]">
              {skill}
            </span>
          ))}
          {skills.length > 5 && (
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          {!version.isDefault ? (
            <button
              ...
              className="tap-target inline-flex items-center gap-1 text-xs font-bold text-[#FF6B2C] hover:text-[#FF6B2C]"
            >
