...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F0] text-[#1A1A1A]">
      ...
      {/* Hero */}
      <section className="relative overflow-hidden border-b border-[#E8E8E4] bg-white">
        ...
        <div className="relative mx-auto max-w-3xl px-5 py-24 text-center sm:px-8">
          <motion.div initial="hidden" animate="show" variants={fadeUp}>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-[#FFCAAF] bg-[#FFE8DC] px-3.5 py-1 text-xs font-semibold text-[#FF6B2C]">
              <Sparkles className="h-3.5 w-3.5" /> AI-Powered Recruitment Platform
            </span>
            <h1 className="mt-5 text-[32px] font-bold leading-[38px] tracking-[-0.6px] text-[#1A1A1A] sm:text-[40px] sm:leading-[48px]">
              Land Your Dream Job with AI-Powered Hiring.
            </h1>
            <p className="mx-auto mt-5 max-w-xl text-[15px] leading-6 text-[#6B6B6B] sm:text-base">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-6 text-sm text-[#6B6B6B]">
              <div className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-[#FF6B2C]" /> Free for candidates, always
              </div>
              <div className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-[#FF6B2C]" /> Apply in minutes
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section id="features" className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
          <h2 className="text-[24px] leading-[30px] font-bold text-[#1A1A1A]">Everything you need to land your next role</h2>
          <p className="mt-3 text-[13px] leading-5 text-[#6B6B6B]">From first upload to your next offer — everything in one place.</p>
        </motion.div>
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map((f, i) => (
            <motion.div key={f.title} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ delay: (i % 4) * 0.06 }}>
              <Card interactive className="group h-full rounded-2xl border border-[#E8E8E4] bg-white p-6 shadow-[0_1px_4px_rgba(27,67,50,0.07)]">
                <IconTile icon={f.icon} tone={i % 4 === 3 ? "ember" : "brand"} className="mb-4" />
                <h3 className="text-[15px] font-semibold text-[#1A1A1A]">{f.title}</h3>
                <p className="mt-2 text-[13px] leading-5 text-[#6B6B6B]">{f.desc}</p>
              </Card>
            </motion.div>
          ))}
        </div>
      ...
      {/* How it works */}
      <section id="how-it-works" className="border-y border-[#E8E8E4] bg-white py-20">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
            <h2 className="text-[24px] leading-[30px] font-bold text-[#1A1A1A]">How it works</h2>
            <p className="mt-3 text-[13px] leading-5 text-[#6B6B6B]">From creating your account to hearing back — a fully connected journey.</p>
          </motion.div>
          <div className="mt-16 flex flex-col gap-0 lg:flex-row lg:items-center lg:justify-between">
            {FLOW.map((step, i) => (
              ...
              >
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#FF6B2C] text-sm font-semibold text-white shadow-[0_1px_4px_rgba(27,67,50,0.07)]">
                  {i + 1}
                </div>
                <p className="text-[13px] font-semibold text-[#1A1A1A] lg:mt-1">{step}</p>
                {i < FLOW.length - 1 && (
      ...
      <section id="why-us" className="mx-auto max-w-7xl px-5 py-24 sm:px-8">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
          <h2 className="text-[24px] leading-[30px] font-bold text-[#1A1A1A]">Why candidates choose AptusHire</h2>
        </motion.div>
          ...
          {WHY_US.map((f, i) => {
            return (
              <motion.div key={f.title} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ delay: (i % 3) * 0.08 }}>
                <Card className="h-full rounded-2xl border border-[#E8E8E4] bg-white p-6 shadow-[0_1px_4px_rgba(27,67,50,0.07)]">
      ...
      {/* About */}
      <section id="about" className="border-y border-[#E8E8E4] bg-white py-20">
        <div className="mx-auto max-w-3xl px-5 text-center sm:px-8">
          <h2 className="text-[24px] leading-[30px] font-bold text-[#1A1A1A]">About AptusHire</h2>
          <p className="mt-4 text-[15px] leading-6 text-[#6B6B6B]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <div className="rounded-2xl border border-[#FFCAAF] bg-[#FFE8DC] px-8 py-14 text-center shadow-[0_1px_4px_rgba(27,67,50,0.07)]">
          <h2 className="text-[24px] leading-[30px] font-bold text-[#1A1A1A]">Ready to start applying?</h2>
          {/* white/90 rather than brand-100: brand-100 on the gradient's
              lightest stop is 4.49:1, which rounds to "fails". */}
          <p className="mx-auto mt-3 max-w-xl text-[13px] leading-5 text-[#6B6B6B]">
          ...
          <div className="flex flex-col justify-between gap-8 sm:flex-row">
            <div>
              <div className="flex items-center gap-2 font-display text-lg font-bold text-slate-900">
                <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#FF6B2C] text-white">
            ...
            <div className="text-sm text-slate-600">
              ...
              <a href="mailto:careers@AptusHire.ai" className="mt-2 flex items-center gap-2 hover:text-[#FF6B2C]">
                <Mail className="h-4 w-4" /> careers@AptusHire.ai
              </a>
              <a href="tel:+911140001234" className="mt-2 flex items-center gap-2 hover:text-[#FF6B2C]">
