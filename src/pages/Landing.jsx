...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-canvas text-text-strong">
      <MarketingNavbar />
      ...
      <section className="relative overflow-hidden border-b border-border bg-surface">
        ...
        <div className="relative mx-auto max-w-3xl px-5 py-24 text-center sm:px-8">
          <motion.div initial="hidden" animate="show" variants={fadeUp}>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-brand-200 bg-brand-50 px-3.5 py-1 text-xs font-semibold text-primary">
              <Sparkles className="h-3.5 w-3.5" /> AI-Powered Recruitment Platform
            </span>
            <h1 className="mt-5 text-[32px] font-bold leading-[38px] tracking-[-0.6px] text-text-strong sm:text-[40px] sm:leading-[48px]">
              Land Your Dream Job with AI-Powered Hiring.
            </h1>
            <p className="mx-auto mt-5 max-w-xl text-[15px] leading-6 text-text-muted sm:text-base">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-6 text-sm text-text-muted">
              <div className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-primary" /> Free for candidates, always
              </div>
              <div className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-primary" /> Apply in minutes
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section id="features" className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
          <h2 className="text-[24px] leading-[30px] font-bold text-text-strong">Everything you need to land your next role</h2>
          <p className="mt-3 text-[13px] leading-5 text-text-muted">From first upload to your next offer — everything in one place.</p>
        </motion.div>
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map((f, i) => (
            <motion.div key={f.title} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ delay: (i % 4) * 0.06 }}>
              <Card interactive className="group h-full rounded-2xl border border-border bg-white p-6 shadow-card">
                <IconTile icon={f.icon} tone={i % 4 === 3 ? "ember" : "brand"} className="mb-4" />
                <h3 className="text-[15px] font-semibold text-text-strong">{f.title}</h3>
                <p className="mt-2 text-[13px] leading-5 text-text-muted">{f.desc}</p>
              </Card>
            </motion.div>
          ))}
        </div>
      ...
      <section id="how-it-works" className="border-y border-border bg-surface py-20">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
            <h2 className="text-[24px] leading-[30px] font-bold text-text-strong">How it works</h2>
            <p className="mt-3 text-[13px] leading-5 text-text-muted">From creating your account to hearing back — a fully connected journey.</p>
          </motion.div>
          <div className="mt-16 flex flex-col gap-0 lg:flex-row lg:items-center lg:justify-between">
            {FLOW.map((step, i) => (
              ...
              >
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-primary text-sm font-semibold text-white shadow-card">
                  {i + 1}
                </div>
                <p className="text-[13px] font-semibold text-text-strong lg:mt-1">{step}</p>
                {i < FLOW.length - 1 && (
      ...
      <section id="why-us" className="mx-auto max-w-7xl px-5 py-24 sm:px-8">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
          <h2 className="text-[24px] leading-[30px] font-bold text-text-strong">Why candidates choose AptusHire</h2>
        </motion.div>
          ...
          {WHY_US.map((f, i) => {
            return (
              <motion.div key={f.title} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ delay: (i % 3) * 0.08 }}>
                <Card className="h-full rounded-2xl border border-border bg-white p-6 shadow-card">
      ...
      {/* About */}
      <section id="about" className="border-y border-border bg-surface py-20">
        <div className="mx-auto max-w-3xl px-5 text-center sm:px-8">
          <h2 className="text-[24px] leading-[30px] font-bold text-text-strong">About AptusHire</h2>
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <div className="rounded-2xl border border-brand-200 bg-brand-50 px-8 py-14 text-center shadow-card">
          <h2 className="text-[24px] leading-[30px] font-bold text-text-strong">Ready to start applying?</h2>
          {/* white/90 rather than brand-100: brand-100 on the gradient's
              lightest stop is 4.49:1, which rounds to "fails". */}
          <p className="mx-auto mt-3 max-w-xl text-[13px] leading-5 text-text-muted">
        ...
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8">
          <div className="flex flex-col justify-between gap-8 sm:flex-row">
            ...
            <div className="text-sm text-slate-600">
              ...
              <a href="mailto:careers@AptusHire.ai" className="mt-2 flex items-center gap-2 hover:text-primary">
                <Mail className="h-4 w-4" /> careers@AptusHire.ai
              </a>
              <a href="tel:+911140001234" className="mt-2 flex items-center gap-2 hover:text-primary">
