...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-[#ECF3EB] text-[#2E2F2D]">
      <MarketingNavbar />

      {/* Hero */}
      <section className="relative overflow-hidden border-b border-[#DFE5DF] bg-[#FBFBFD]">
        ...
        <div className="relative mx-auto max-w-3xl px-5 py-24 text-center sm:px-8">
          <motion.div initial="hidden" animate="show" variants={fadeUp}>
            <span className="inline-flex items-center gap-1.5 rounded-full border border-[#D2ECC9] bg-[#EAF9E1] px-3.5 py-1 text-xs font-semibold text-[#214740]">
              <Sparkles className="h-3.5 w-3.5" /> AI-Powered Recruitment Platform
            </span>
            <h1 className="mt-5 text-[32px] font-bold leading-[38px] tracking-[-0.6px] text-[#2E2F2D] sm:text-[40px] sm:leading-[48px]">
              Land Your Dream Job with AI-Powered Hiring.
            </h1>
            <p className="mx-auto mt-5 max-w-xl text-[15px] leading-6 text-[#5A7B71] sm:text-base">
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-6 text-sm text-[#5A7B71]">
              <div className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-[#214740]" /> Free for candidates, always
              </div>
              <div className="flex items-center gap-1.5 font-medium">
                <CheckCircle2 className="h-4 w-4 text-[#214740]" /> Apply in minutes
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <section id="features" className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
          <h2 className="text-[24px] leading-[30px] font-bold text-[#2E2F2D]">Everything you need to land your next role</h2>
          <p className="mt-3 text-[13px] leading-5 text-[#5A7B71]">From first upload to your next offer — everything in one place.</p>
        </motion.div>
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {FEATURES.map((f, i) => (
            <motion.div key={f.title} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ delay: (i % 4) * 0.06 }}>
              <Card interactive className="group h-full rounded-2xl border border-[#DFE5DF] bg-white p-6 shadow-card">
                <IconTile icon={f.icon} tone={i % 4 === 3 ? "ember" : "brand"} className="mb-4" />
                <h3 className="text-[15px] font-semibold text-[#2E2F2D]">{f.title}</h3>
                <p className="mt-2 text-[13px] leading-5 text-[#707E79]">{f.desc}</p>
              </Card>
            </motion.div>
          ))}
        </div>
      ...
      {/* How it works */}
      <section id="how-it-works" className="border-y border-[#DFE5DF] bg-[#FBFBFD] py-20">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
            <h2 className="text-[24px] leading-[30px] font-bold text-[#2E2F2D]">How it works</h2>
            <p className="mt-3 text-[13px] leading-5 text-[#5A7B71]">From creating your account to hearing back — a fully connected journey.</p>
          </motion.div>
          <div className="mt-16 flex flex-col gap-0 lg:flex-row lg:items-center lg:justify-between">
            {FLOW.map((step, i) => (
              ...
              >
                <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-[#214740] text-sm font-semibold text-white shadow-card">
                  {i + 1}
                </div>
                <p className="text-[13px] font-semibold text-[#2E2F2D] lg:mt-1">{step}</p>
                {i < FLOW.length - 1 && (
      ...
      <section id="why-us" className="mx-auto max-w-7xl px-5 py-24 sm:px-8">
        <motion.div initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="mx-auto max-w-2xl text-center">
          <h2 className="text-[24px] leading-[30px] font-bold text-[#2E2F2D]">Why candidates choose AptusHire</h2>
        </motion.div>
          ...
          {WHY_US.map((f, i) => {
            return (
              <motion.div key={f.title} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} transition={{ delay: (i % 3) * 0.08 }}>
                <Card className="h-full rounded-2xl border border-[#DFE5DF] bg-white p-6 shadow-card">
      ...
      {/* About */}
      <section id="about" className="border-y border-[#DFE5DF] bg-[#FBFBFD] py-20">
        <div className="mx-auto max-w-3xl px-5 text-center sm:px-8">
          <h2 className="text-[24px] leading-[30px] font-bold text-[#2E2F2D]">About AptusHire</h2>
          <p className="mt-4 text-[15px] leading-6 text-[#707E79]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-20 sm:px-8">
        <div className="rounded-2xl border border-[#D2ECC9] bg-[#EAF9E1] px-8 py-14 text-center shadow-card">
          <h2 className="text-[24px] leading-[30px] font-bold text-[#2E2F2D]">Ready to start applying?</h2>
          {/* white/90 rather than brand-100: brand-100 on the gradient's
              lightest stop is 4.49:1, which rounds to "fails". */}
          <p className="mx-auto mt-3 max-w-xl text-[13px] leading-5 text-[#5A7B71]">
        ...
        <div className="mx-auto max-w-7xl px-5 py-12 sm:px-8">
          <div className="flex flex-col justify-between gap-8 sm:flex-row">
            ...
            <div className="text-sm text-slate-600">
              ...
              <a href="mailto:careers@AptusHire.ai" className="mt-2 flex items-center gap-2 hover:text-brand-700">
