  ...
  return (
    <Card className="border-[#E5EBE7] bg-white dark:border-[#E5EBE7] dark:bg-white">
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${upcoming ? "bg-[#E8F2EC] text-[#176B45]" : "bg-[#F3F5F3] text-[#64736A]"}`}><Video className="h-5 w-5" /></span><div className="min-w-0"><h3 className="truncate text-[15px] font-semibold text-[#17221C]">{title}</h3><p className="mt-1 truncate text-xs text-[#64736A]">{company || "Company unavailable"}</p></div></div>
        <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold ${upcoming ? "bg-[#E8F2EC] text-[#176B45]" : "bg-[#F3F5F3] text-[#64736A]"}`}>{status}</span>
      </div>
      <div className="mt-4 grid gap-2 text-xs text-[#64736A] sm:grid-cols-2"><span className="inline-flex items-center gap-1.5"><CalendarDays className="h-4 w-4" />{dateTime(interview.interviewAt)}</span><span className="inline-flex items-center gap-1.5"><Clock3 className="h-4 w-4" />{type}</span></div>
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#E5EBE7] pt-4"><span className="text-xs text-[#64736A]">{upcoming ? "Use your secure invitation to join." : "Your interview responses were recorded."}</span>{upcoming && <Button type="button" size="sm" loading={opening} onClick={() => onOpen(interview)}><PlayCircle className="h-3.5 w-3.5" /> Join interview</Button>}</div>
    </Card>
  );
}
...
export default function Interviews() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 pb-10">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#64736A]">My progress</p><h1 className="mt-2 text-2xl font-bold text-[#17221C]">Interviews</h1><p className="mt-2 text-sm text-[#64736A]">Keep track of scheduled and completed interviews.</p></div><Button type="button" variant="outline" size="sm" onClick={load}><RotateCcw className="h-3.5 w-3.5" /> Refresh</Button></div>
