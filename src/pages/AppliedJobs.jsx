        ...
        {TRACK_STAGES.map((stage, index) => {
          ...
          return (
            <div key={stage.key} className="flex min-w-0 flex-1 items-center">
              <div className={`relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 text-[11px] font-bold ${complete ? "border-[#214740] bg-primary text-white" : "border-border bg-white text-text-muted"}`}>
                {complete ? <Check className="h-3.5 w-3.5" aria-hidden="true" /> : index + 1}
                {active && <span className="absolute -inset-1 rounded-full border border-brand-200" aria-hidden="true" />}
              </div>
              {index < TRACK_STAGES.length - 1 && <div className={`h-0.5 min-w-3 flex-1 ${!rejected && current > index ? "bg-primary" : "bg-border"}`} />}
            </div>
          );
        })}
      </div>
      <div className="mt-2 grid grid-cols-4 text-[11px] font-medium text-text-muted">
        {TRACK_STAGES.map((stage, index) => <span key={stage.key} className={index === current ? "font-bold text-primary" : ""}>{stage.label}</span>)}
      </div>
      {rejected && <p className="mt-2 text-xs font-semibold text-verdict-negative">This application was rejected.</p>}
    </div>
  );
}
...
function ApplicationTimeline({ status, stageHistory = [] }) {
  ...
  return (
    <div className="mt-7 border-t border-border pt-6">
      <h3 className="text-[15px] font-semibold text-text-strong">Application timeline</h3>
        ...
        {events.map((event, index) => {
          ...
          return (
            <li key={event.key} className="relative flex gap-3 pb-5 last:pb-0">
              {index < events.length - 1 && <span className={`absolute left-[9px] top-5 h-full w-px ${completed ? "bg-primary" : "bg-border"}`} aria-hidden="true" />}
              <span className={`relative z-10 mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[11px] font-bold ${completed ? "border-[#214740] bg-primary text-white" : active ? "border-[#214740] bg-brand-50 text-primary" : "border-border bg-white text-text-muted"}`}>
                {completed ? <Check className="h-3 w-3" aria-hidden="true" /> : active ? "●" : "○"}
              </span>
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
                  <p className={`text-[13px] font-semibold ${pending ? "text-text-muted" : "text-text-strong"}`}>{event.label}</p>
                  <span className="text-[11px] text-text-muted">{dateTimeLabel(event.entry?.at)}</span>
                </div>
                <p className={`mt-0.5 text-[11px] ${active ? "font-semibold text-primary" : "text-text-muted"}`}>
...
function ApplicationDetails({ application, loading, error, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-primary/45 p-4" role="presentation" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-labelledby="application-details-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-border bg-white p-6 shadow-xl sm:p-8" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.1em] text-text-muted">Application details</p>
            <h2 id="application-details-title" className="mt-1 text-xl font-bold text-text-strong">{job?.title || "Application"}</h2>
            <p className="mt-1 text-sm text-text-muted">{job?.company?.name || "Company unavailable"}</p>
          </div>
          <button type="button" onClick={onClose} aria-label="Close application details" className="tap-target inline-flex h-9 w-9 items-center justify-center rounded-lg text-text-muted hover:bg-brand-50 hover:text-primary"><X className="h-5 w-5" /></button>
        </div>
        {loading && <p className="mt-6 rounded-xl bg-surface p-4 text-sm text-text-muted">Loading your application details...</p>}
        {error && <p role="alert" className="mt-6 rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</p>}
        <div className="mt-6 grid gap-4 border-y border-border py-4 sm:grid-cols-3">
          <div><p className="text-[11px] font-medium uppercase text-text-muted">Status</p><p className="mt-1 text-sm font-semibold text-primary">{stageLabel(status)}</p></div>
          <div><p className="text-[11px] font-medium uppercase text-text-muted">Applied</p><p className="mt-1 text-sm font-semibold text-text-strong">{dateLabel(createdAt)}</p></div>
          <div><p className="text-[11px] font-medium uppercase text-text-muted">Resume</p><p className="mt-1 break-words text-sm font-semibold text-text-strong">{resumeOriginalName || "Resume on file"}</p></div>
        </div>
        <ProgressTracker status={status} />
        <ApplicationTimeline status={status} stageHistory={stageHistory} />
        {application.basicDetails && <div className="mt-7"><h3 className="text-[15px] font-semibold text-text-strong">Submitted information</h3><div className="mt-3 grid gap-3 text-sm sm:grid-cols-2">{Object.entries(application.basicDetails).filter(([, value]) => value).map(([key, value]) => <div key={key}><p className="text-[11px] font-medium uppercase text-text-muted">{key.replace(/([A-Z])/g, " $1")}</p><p className="mt-1 break-words text-text-strong">{value}</p></div>)}</div></div>}
        {application.education?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-text-strong">Education</h3><div className="mt-3 space-y-2">{application.education.map((entry, index) => <div key={entry._id || index} className="rounded-xl bg-surface p-3 text-sm"><p className="font-semibold text-text-strong">{entry.degree}{entry.fieldOfStudy ? `, ${entry.fieldOfStudy}` : ""}</p><p className="mt-1 text-text-muted">{entry.institution}{entry.endYear ? ` · ${entry.endYear}` : ""}</p></div>)}</div></div>}
        {application.skills?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-text-strong">Skills</h3><div className="mt-3 flex flex-wrap gap-2">{application.skills.map((skill) => <span key={skill} className="rounded-full bg-brand-50 px-3 py-1 text-xs font-semibold text-primary">{skill}</span>)}</div></div>}
        {application.experience?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-text-strong">Experience</h3><div className="mt-3 space-y-2">{application.experience.map((entry, index) => <div key={entry._id || index} className="rounded-xl bg-surface p-3 text-sm"><p className="font-semibold text-text-strong">{entry.role} · {entry.company}</p><p className="mt-1 text-text-muted">{entry.startDate || ""}{entry.endDate ? ` - ${entry.endDate}` : ""}</p>{entry.description && <p className="mt-2 whitespace-pre-line text-text-muted">{entry.description}</p>}</div>)}</div></div>}
        {stageHistory.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-text-strong">Status history</h3><div className="mt-3 space-y-2">{stageHistory.map((entry, index) => <div key={`${entry.stage}-${entry.at || index}`} className="flex items-center justify-between gap-3 rounded-xl bg-surface px-3 py-2 text-xs"><span className="font-semibold text-text-strong">{stageLabel(entry.stage)}</span><span className="text-text-muted">{dateLabel(entry.at)}</span></div>)}</div></div>}
        {offer?.status && <p className="mt-6 rounded-xl bg-brand-50 p-3 text-sm font-semibold text-primary">Offer status: {offer.status}</p>}
...
function ApplicationRow({ application, onView }) {
  ...
  return (
    <tr className="group border-t border-border text-[13px] text-text-muted transition-colors hover:bg-[#FBFCFB]">
...
export default function AppliedJobs() {
  ...
  return (
    <div className="mx-auto max-w-[1120px] space-y-5 pb-10">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-text-muted">Your activity</p><h1 className="mt-2 text-2xl font-bold text-text-strong">Applied Jobs</h1><p className="mt-2 text-sm text-text-muted">A list of the jobs you've applied to</p></div><label className="flex min-h-11 items-center gap-2 rounded-full border border-[#E0E5E2] bg-white px-4 text-sm text-[#77807D] shadow-[0_2px_6px_rgba(33,71,64,.06)] focus-within:border-[#A7D68E] focus-within:ring-2 focus-within:ring-[#EAF9E1]"><Search className="h-4 w-4" /><span className="sr-only">Search applications</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search" className="w-32 border-0 bg-transparent p-0 text-sm outline-none placeholder:text-[#77807D] focus:ring-0 sm:w-40" /></label></div>
      ...
      {state === "ready" && sortedApplications.length > 0 && <><p className="text-[13px] text-text-muted"><span className="font-semibold text-text-strong">{filteredApplications.length}</span> {filteredApplications.length === 1 ? "application" : "applications"}</p>{filteredApplications.length > 0 ? <div className="overflow-x-auto rounded-[16px] border border-border bg-white shadow-[0_5px_18px_rgba(33,71,64,.05)]"><table className="w-full min-w-[760px] border-collapse text-left"><thead className="bg-[#FBFCFB]"><tr className="text-[11px] font-bold text-[#14233A]"><th className="px-3 py-3 sm:px-4">Company</th><th className="px-3 py-3 sm:px-4">Job title</th><th className="px-3 py-3 sm:px-4">Type</th><th className="px-3 py-3 sm:px-4">Date Applied</th><th className="px-3 py-3 sm:px-4">Status</th><th className="px-2 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody>{filteredApplications.map((application) => <ApplicationRow key={application._id} application={application} onView={viewApplication} />)}</tbody></table></div> : <Card><p className="text-sm text-text-muted">No applications match &quot;{query}&quot;.</p></Card>}</>}
