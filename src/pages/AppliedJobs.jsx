        ...
        {TRACK_STAGES.map((stage, index) => {
          ...
          return (
            <div key={stage.key} className="flex min-w-0 flex-1 items-center">
              ...
              {index < TRACK_STAGES.length - 1 && <div className={`h-0.5 min-w-3 flex-1 ${!rejected && current > index ? "bg-[#176B45]" : "bg-border"}`} />}
            </div>
          );
        })}
      </div>
      <div className="mt-2 grid grid-cols-4 text-[11px] font-medium text-[#64736A]">
        {TRACK_STAGES.map((stage, index) => <span key={stage.key} className={index === current ? "font-bold text-[#176B45]" : ""}>{stage.label}</span>)}
      </div>
      {rejected && <p className="mt-2 text-xs font-semibold text-[#C95C5C]">This application was rejected.</p>}
    </div>
  );
}
...
function ApplicationTimeline({ status, stageHistory = [] }) {
  ...
  return (
    <div className="mt-7 border-t border-[#E5EBE7] pt-6">
      <h3 className="text-[15px] font-semibold text-[#17221C]">Application timeline</h3>
            ...
            <li key={event.key} className="relative flex gap-3 pb-5 last:pb-0">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
                  <p className={`text-[13px] font-semibold ${pending ? "text-[#64736A]" : "text-[#17221C]"}`}>{event.label}</p>
                  <span className="text-[11px] text-[#64736A]">{dateTimeLabel(event.entry?.at)}</span>
                </div>
                <p className={`mt-0.5 text-[11px] ${active ? "font-semibold text-[#176B45]" : "text-[#64736A]"}`}>
...
function ApplicationDetails({ application, loading, error, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#176B45]/45 p-4" role="presentation" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-labelledby="application-details-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-[#E5EBE7] bg-white p-6 shadow-xl sm:p-8" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.1em] text-[#64736A]">Application details</p>
            <h2 id="application-details-title" className="mt-1 text-xl font-bold text-[#17221C]">{job?.title || "Application"}</h2>
            <p className="mt-1 text-sm text-[#64736A]">{job?.company?.name || "Company unavailable"}</p>
          </div>
          <button type="button" onClick={onClose} aria-label="Close application details" className="tap-target inline-flex h-9 w-9 items-center justify-center rounded-lg text-[#64736A] hover:bg-[#DDECE3] hover:text-[#176B45]"><X className="h-5 w-5" /></button>
        </div>
        {loading && <p className="mt-6 rounded-xl bg-white p-4 text-sm text-[#64736A]">Loading your application details...</p>}
        {error && <p role="alert" className="mt-6 rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</p>}
        <div className="mt-6 grid gap-4 border-y border-[#E5EBE7] py-4 sm:grid-cols-3">
          <div><p className="text-[11px] font-medium uppercase text-[#64736A]">Status</p><p className="mt-1 text-sm font-semibold text-[#176B45]">{stageLabel(status)}</p></div>
          <div><p className="text-[11px] font-medium uppercase text-[#64736A]">Applied</p><p className="mt-1 text-sm font-semibold text-[#17221C]">{dateLabel(createdAt)}</p></div>
          <div><p className="text-[11px] font-medium uppercase text-[#64736A]">Resume</p><p className="mt-1 break-words text-sm font-semibold text-[#17221C]">{resumeOriginalName || "Resume on file"}</p></div>
        </div>
        <ProgressTracker status={status} />
        <ApplicationTimeline status={status} stageHistory={stageHistory} />
        {application.basicDetails && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#17221C]">Submitted information</h3><div className="mt-3 grid gap-3 text-sm sm:grid-cols-2">{Object.entries(application.basicDetails).filter(([, value]) => value).map(([key, value]) => <div key={key}><p className="text-[11px] font-medium uppercase text-[#64736A]">{key.replace(/([A-Z])/g, " $1")}</p><p className="mt-1 break-words text-[#17221C]">{value}</p></div>)}</div></div>}
        {application.education?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#17221C]">Education</h3><div className="mt-3 space-y-2">{application.education.map((entry, index) => <div key={entry._id || index} className="rounded-xl bg-white p-3 text-sm"><p className="font-semibold text-[#17221C]">{entry.degree}{entry.fieldOfStudy ? `, ${entry.fieldOfStudy}` : ""}</p><p className="mt-1 text-[#64736A]">{entry.institution}{entry.endYear ? ` · ${entry.endYear}` : ""}</p></div>)}</div></div>}
        {application.skills?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#17221C]">Skills</h3><div className="mt-3 flex flex-wrap gap-2">{application.skills.map((skill) => <span key={skill} className="rounded-full bg-[#E8F2EC] px-3 py-1 text-xs font-semibold text-[#176B45]">{skill}</span>)}</div></div>}
        {application.experience?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#17221C]">Experience</h3><div className="mt-3 space-y-2">{application.experience.map((entry, index) => <div key={entry._id || index} className="rounded-xl bg-white p-3 text-sm"><p className="font-semibold text-[#17221C]">{entry.role} · {entry.company}</p><p className="mt-1 text-[#64736A]">{entry.startDate || ""}{entry.endDate ? ` - ${entry.endDate}` : ""}</p>{entry.description && <p className="mt-2 whitespace-pre-line text-[#64736A]">{entry.description}</p>}</div>)}</div></div>}
        {stageHistory.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#17221C]">Status history</h3><div className="mt-3 space-y-2">{stageHistory.map((entry, index) => <div key={`${entry.stage}-${entry.at || index}`} className="flex items-center justify-between gap-3 rounded-xl bg-white px-3 py-2 text-xs"><span className="font-semibold text-[#17221C]">{stageLabel(entry.stage)}</span><span className="text-[#64736A]">{dateLabel(entry.at)}</span></div>)}</div></div>}
        {offer?.status && <p className="mt-6 rounded-xl bg-[#E8F2EC] p-3 text-sm font-semibold text-[#176B45]">Offer status: {offer.status}</p>}
...
function ApplicationRow({ application, onView }) {
  ...
  return (
    <tr className="group border-t border-[#E5EBE7] text-[13px] text-[#64736A] transition-colors hover:bg-[#FBFCFB]">
...
export default function AppliedJobs() {
  ...
  return (
    <div className="mx-auto max-w-[1120px] space-y-5 pb-10">
      ...
      {state === "ready" && sortedApplications.length > 0 && <><p className="text-[13px] text-[#64736A]"><span className="font-semibold text-[#17221C]">{filteredApplications.length}</span> {filteredApplications.length === 1 ? "application" : "applications"}</p>{filteredApplications.length > 0 ? <div className="overflow-x-auto rounded-[16px] border border-[#E5EBE7] bg-white shadow-[0_5px_18px_rgba(33,71,64,.05)]"><table className="w-full min-w-[760px] border-collapse text-left"><thead className="bg-[#FBFCFB]"><tr className="text-[11px] font-bold text-[#14233A]"><th className="px-3 py-3 sm:px-4">Company</th><th className="px-3 py-3 sm:px-4">Job title</th><th className="px-3 py-3 sm:px-4">Type</th><th className="px-3 py-3 sm:px-4">Date Applied</th><th className="px-3 py-3 sm:px-4">Status</th><th className="px-2 py-3"><span className="sr-only">Actions</span></th></tr></thead><tbody>{filteredApplications.map((application) => <ApplicationRow key={application._id} application={application} onView={viewApplication} />)}</tbody></table></div> : <Card><p className="text-sm text-[#64736A]">No applications match &quot;{query}&quot;.</p></Card>}</>}
