        ...
        {TRACK_STAGES.map((stage, index) => {
          ...
          return (
            <div key={stage.key} className="flex min-w-0 flex-1 items-center">
              <div className={`relative z-10 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border-2 text-[11px] font-bold ${complete ? "border-[#214740] bg-[#F97316] text-white" : "border-[#E2E8F0] bg-white text-[#64748B]"}`}>
                {complete ? <Check className="h-3.5 w-3.5" aria-hidden="true" /> : index + 1}
                {active && <span className="absolute -inset-1 rounded-full border border-[#FED7AA]" aria-hidden="true" />}
              </div>
              {index < TRACK_STAGES.length - 1 && <div className={`h-0.5 min-w-3 flex-1 ${!rejected && current > index ? "bg-[#F97316]" : "bg-border"}`} />}
            </div>
          );
        })}
      </div>
      <div className="mt-2 grid grid-cols-4 text-[11px] font-medium text-[#64748B]">
        {TRACK_STAGES.map((stage, index) => <span key={stage.key} className={index === current ? "font-bold text-[#F97316]" : ""}>{stage.label}</span>)}
      </div>
      {rejected && <p className="mt-2 text-xs font-semibold text-[#DC2626]">This application was rejected.</p>}
    </div>
  );
}
...
function ApplicationTimeline({ status, stageHistory = [] }) {
  ...
  return (
    <div className="mt-7 border-t border-[#E2E8F0] pt-6">
      <h3 className="text-[15px] font-semibold text-[#0F172A]">Application timeline</h3>
        ...
        {events.map((event, index) => {
          ...
          return (
            <li key={event.key} className="relative flex gap-3 pb-5 last:pb-0">
              {index < events.length - 1 && <span className={`absolute left-[9px] top-5 h-full w-px ${completed ? "bg-[#F97316]" : "bg-border"}`} aria-hidden="true" />}
              <span className={`relative z-10 mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[11px] font-bold ${completed ? "border-[#214740] bg-[#F97316] text-white" : active ? "border-[#214740] bg-[#FEF3E8] text-[#F97316]" : "border-[#E2E8F0] bg-white text-[#64748B]"}`}>
                {completed ? <Check className="h-3 w-3" aria-hidden="true" /> : active ? "●" : "○"}
              </span>
              <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-baseline justify-between gap-x-3 gap-y-1">
                  <p className={`text-[13px] font-semibold ${pending ? "text-[#64748B]" : "text-[#0F172A]"}`}>{event.label}</p>
                  <span className="text-[11px] text-[#64748B]">{dateTimeLabel(event.entry?.at)}</span>
                </div>
                <p className={`mt-0.5 text-[11px] ${active ? "font-semibold text-[#F97316]" : "text-[#64748B]"}`}>
...
function ApplicationDetails({ application, loading, error, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#F97316]/45 p-4" role="presentation" onMouseDown={onClose}>
      <section role="dialog" aria-modal="true" aria-labelledby="application-details-title" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-[#E2E8F0] bg-white p-6 shadow-xl sm:p-8" onMouseDown={(event) => event.stopPropagation()}>
        <div className="flex items-start justify-between gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.1em] text-[#64748B]">Application details</p>
        ...
        <div className="mt-6 grid gap-4 border-y border-[#E2E8F0] py-4 sm:grid-cols-3">
          <div><p className="text-[11px] font-medium uppercase text-[#64748B]">Status</p><p className="mt-1 text-sm font-semibold text-[#F97316]">{stageLabel(status)}</p></div>
          <div><p className="text-[11px] font-medium uppercase text-[#64748B]">Applied</p><p className="mt-1 text-sm font-semibold text-[#0F172A]">{dateLabel(createdAt)}</p></div>
          <div><p className="text-[11px] font-medium uppercase text-[#64748B]">Resume</p><p className="mt-1 break-words text-sm font-semibold text-[#0F172A]">{resumeOriginalName || "Resume on file"}</p></div>
        </div>
        <ProgressTracker status={status} />
        <ApplicationTimeline status={status} stageHistory={stageHistory} />
        {application.basicDetails && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#0F172A]">Submitted information</h3><div className="mt-3 grid gap-3 text-sm sm:grid-cols-2">{Object.entries(application.basicDetails).filter(([, value]) => value).map(([key, value]) => <div key={key}><p className="text-[11px] font-medium uppercase text-[#64748B]">{key.replace(/([A-Z])/g, " $1")}</p><p className="mt-1 break-words text-[#0F172A]">{value}</p></div>)}</div></div>}
        {application.education?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#0F172A]">Education</h3><div className="mt-3 space-y-2">{application.education.map((entry, index) => <div key={entry._id || index} className="rounded-xl bg-white p-3 text-sm"><p className="font-semibold text-[#0F172A]">{entry.degree}{entry.fieldOfStudy ? `, ${entry.fieldOfStudy}` : ""}</p><p className="mt-1 text-[#64748B]">{entry.institution}{entry.endYear ? ` · ${entry.endYear}` : ""}</p></div>)}</div></div>}
        {application.skills?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#0F172A]">Skills</h3><div className="mt-3 flex flex-wrap gap-2">{application.skills.map((skill) => <span key={skill} className="rounded-full bg-[#FEF3E8] px-3 py-1 text-xs font-semibold text-[#F97316]">{skill}</span>)}</div></div>}
        {application.experience?.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#0F172A]">Experience</h3><div className="mt-3 space-y-2">{application.experience.map((entry, index) => <div key={entry._id || index} className="rounded-xl bg-white p-3 text-sm"><p className="font-semibold text-[#0F172A]">{entry.role} · {entry.company}</p><p className="mt-1 text-[#64748B]">{entry.startDate || ""}{entry.endDate ? ` - ${entry.endDate}` : ""}</p>{entry.description && <p className="mt-2 whitespace-pre-line text-[#64748B]">{entry.description}</p>}</div>)}</div></div>}
        {stageHistory.length > 0 && <div className="mt-7"><h3 className="text-[15px] font-semibold text-[#0F172A]">Status history</h3><div className="mt-3 space-y-2">{stageHistory.map((entry, index) => <div key={`${entry.stage}-${entry.at || index}`} className="flex items-center justify-between gap-3 rounded-xl bg-white px-3 py-2 text-xs"><span className="font-semibold text-[#0F172A]">{stageLabel(entry.stage)}</span><span className="text-[#64748B]">{dateLabel(entry.at)}</span></div>)}</div></div>}
        {offer?.status && <p className="mt-6 rounded-xl bg-[#FEF3E8] p-3 text-sm font-semibold text-[#F97316]">Offer status: {offer.status}</p>}
