    ...
    <div className="space-y-5 pb-8 text-[#14233A]">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {versions.length > 0 && <select aria-label="Select CV for evaluation" value={resume?._id || ""} onChange={(event) => setResume(versions.find((version) => String(version._id) === event.target.value) || null)} className="min-h-11 max-w-full rounded-full border border-[#DDE8DE] bg-white px-3 text-sm text-primary">
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploading} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-primary px-5 text-sm font-semibold text-white shadow-[0_7px_16px_rgba(33,71,64,.15)] hover:bg-primary-dark disabled:opacity-60"><UploadCloud className="h-4 w-4" /> {uploading ? "Uploading…" : "Upload CV"}</button>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-[14px] border border-[#DDE8DE] bg-[#F5FAF2] px-4 py-3 text-sm text-text-muted">
        <ShieldCheck className="h-5 w-5 text-primary" />
        <span>{resume ? <>Evaluating <strong className="text-primary">{resume.label || "your default CV"}</strong></> : "Add a CV to generate your evaluation"}</span>
      ...
      {analysis ? (
        <>
          ...
          <section className="rounded-[18px] border border-[#DFE5E4] bg-white p-5 shadow-[0_5px_18px_rgba(33,71,64,.06)] sm:p-6">
            <div className="mb-4 flex items-center gap-2"><FileSearch className="h-5 w-5 text-primary" /><h2 className="text-[20px] font-semibold text-[#252B29]">Professional Profile Analysis</h2></div>
