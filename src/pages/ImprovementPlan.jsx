...
export default function ImprovementPlan() {
  ...
  if (state === "loading") return <Card className="mx-auto max-w-3xl text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-[#176B45]" /><p className="mt-3 text-sm text-[#3F4A45]">Loading your improvement plan...</p></Card>;
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 pb-10">
      ...
      <div><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#4A554F]">Application update</p><h1 className="mt-2 text-2xl font-bold text-[#17221C]">Your Improvement Plan</h1><p className="mt-2 text-sm text-[#3F4A45]">You can improve your next application.</p><p className="mt-2 text-sm text-[#3F4A45]">{application.job?.title || "Application"} · {stageLabel(application.status)}</p></div>
      {report && <>
        <Card><h2 className="text-[15px] font-semibold text-[#17221C]">Why you were not selected</h2><p className="mt-2 text-sm text-[#3F4A45]">{report.summary}</p></Card>
      ...
      {report?.improvementPlan?.map((phase) => <Card key={phase.period}><h2 className="text-[15px] font-semibold text-[#17221C]">{phase.period}</h2><ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-[#3F4A45]">{phase.actions?.map((action) => <li key={action}>{action}</li>)}</ul></Card>)}
    </div>
  );
}
