...
function ImprovementArea({ item }) {
  return (
    <Card className="border-[#E2E8F0] bg-white dark:border-[#E2E8F0] dark:bg-white">
      <div className="flex items-start justify-between gap-4"><div className="flex min-w-0 items-center gap-3"><span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#FEF3E8] text-[#F97316]"><Target className="h-5 w-5" /></span><h2 className="truncate text-[15px] font-semibold text-[#0F172A]">{item.label}</h2></div>{item.current != null && <span className="shrink-0 text-lg font-bold text-[#F97316]">{item.current}%</span>}</div>
      <dl className="mt-5 grid gap-4 border-t border-[#E2E8F0] pt-4 sm:grid-cols-2"><div><dt className="text-xs font-semibold uppercase text-[#4A554F]">Current level</dt><dd className="mt-1 text-sm font-semibold text-[#0F172A]">{item.current == null ? "Not available" : `${item.current}%`}</dd></div><div><dt className="text-xs font-semibold uppercase text-[#4A554F]">Target level</dt><dd className="mt-1 text-sm font-semibold text-[#3F4A45]">{item.target == null ? "Not provided" : `${item.target}%`}</dd></div><div><dt className="text-xs font-semibold uppercase text-[#4A554F]">Why it matters</dt><dd className="mt-1 text-sm text-[#3F4A45]">{item.why || "No rationale is stored for this improvement area."}</dd></div><div><dt className="text-xs font-semibold uppercase text-[#4A554F]">Recommended action</dt><dd className="mt-1 text-sm text-[#3F4A45]">{item.action || "No recommendation is available yet."}</dd></div></dl>
...
export default function ImprovementPlan() {
  ...
  if (state === "loading") return <Card className="mx-auto max-w-3xl text-center"><Loader2 className="mx-auto h-6 w-6 animate-spin text-[#F97316]" /><p className="mt-3 text-sm text-[#3F4A45]">Loading your improvement plan...</p></Card>;
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6 pb-10">
      ...
      {report && <>
        ...
        <Card><h2 className="text-[15px] font-semibold text-[#0F172A]">Rejection reasons</h2><div className="mt-4 space-y-3">{report.rejectionReasons?.map((reason) => <div key={`${reason.requirement}-${reason.reason}`} className="border-b border-[#E2E8F0] pb-3 last:border-0"><p className="text-sm font-semibold text-[#0F172A]">{reason.reason}</p><p className="mt-1 text-sm text-[#3F4A45]">{reason.severity} · {reason.evidence}</p></div>)}</div></Card>
        <Card><h2 className="text-[15px] font-semibold text-[#0F172A]">Job requirement alignment</h2><div className="mt-4 space-y-3">{report.requirementAnalysis?.map((item) => <div key={item.requirement} className="grid gap-1 border-b border-[#E2E8F0] pb-3 last:border-0 sm:grid-cols-[1fr_auto]"><div><p className="text-sm font-semibold text-[#0F172A]">{item.requirement}</p><p className="mt-1 text-sm text-[#3F4A45]">{item.evidence}</p></div><span className="text-sm font-semibold text-[#F97316]">{item.status}</span></div>)}</div></Card>
        <Card><h2 className="text-[15px] font-semibold text-[#0F172A]">Assessment and interview</h2><p className="mt-2 text-sm text-[#3F4A45]">Assessment: {report.assessmentAnalysis?.overallScore == null ? report.assessmentAnalysis?.status : `${report.assessmentAnalysis.overallScore}%`}</p><p className="mt-1 text-sm text-[#3F4A45]">Interview: {report.interviewAnalysis?.overallScore == null ? report.interviewAnalysis?.status : `${report.interviewAnalysis.overallScore}%`}</p></Card>
      </>}
      <Card className="border-[#E2E8F0] bg-white dark:border-[#E2E8F0] dark:bg-white"><div className="flex items-start gap-3"><Lightbulb className="mt-0.5 h-5 w-5 shrink-0 text-[#F97316]" /><div><h2 className="text-[15px] font-semibold text-[#0F172A]">Improvement areas</h2><p className="mt-1 text-sm text-[#3F4A45]">Recommendations below are based on the recorded evidence for this application.</p></div></div></Card>
      {areas.length > 0 ? <div className="grid gap-4 md:grid-cols-2">{areas.map((area) => <ImprovementArea key={area.label} item={area} />)}</div> : <Card><p className="text-sm text-[#3F4A45]">No assessment performance data is available for this application.</p></Card>}
      {report?.improvementPlan?.map((phase) => <Card key={phase.period}><h2 className="text-[15px] font-semibold text-[#0F172A]">{phase.period}</h2><ul className="mt-3 list-disc space-y-1 pl-5 text-sm text-[#3F4A45]">{phase.actions?.map((action) => <li key={action}>{action}</li>)}</ul></Card>)}
    </div>
  );
}
