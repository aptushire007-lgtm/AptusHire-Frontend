...
const STATUS = {
  scheduled: { label: "Invited", tone: "bg-[#EAF9E1] text-[#214740]" },
...
function AssessmentGroup({ title, items, onOpen, openingId }) {
  ...
  return <section><h2 className="mb-3 text-[16px] font-semibold text-[#2E2F2D]">{title}</h2><div className="grid gap-4 md:grid-cols-2">{items.map((assessment) => <AssessmentCard key={assessment._id} assessment={assessment} onOpen={onOpen} opening={openingId === assessment._id} />)}</div></section>;
}
...
function AssessmentRow({ assessment, onOpen, opening }) {
  ...
  const active = ["in_progress", "paused"].includes(String(assessment.status || "").toLowerCase());
  const date = completed ? assessment.completedAt : assessment.validFrom || assessment.startedAt || assessment.createdAt;
  return <tr className="border-t border-[#EDF1ED] text-sm text-[#2E2F2D] hover:bg-[#FBFCFB]"><td className="break-words px-3 py-4 font-semibold sm:px-4">{assessment.job?.company?.name || "Company"}</td><td className="break-words px-3 py-4 sm:px-4"><p className="break-words font-medium">{assessment.title || assessment.job?.title || "Assessment"}</p><p className="mt-1 break-words text-xs text-[#707E79]">{assessment.job?.title || "Related job"}</p></td><td className="break-words px-3 py-4 sm:px-4"><span className="rounded-md bg-[#EEF0FF] px-2 py-1 text-[11px] font-semibold text-[#5A62D6]">Skills assessment</span></td><td className="break-words px-3 py-4 text-xs text-[#707E79] sm:px-4">{dateTime(date)}{assessment.expiresAt && <span className="mt-1 block">Due {dateTime(assessment.expiresAt)}</span>}</td><td className="break-words px-3 py-4 sm:px-4"><span className={`inline-flex whitespace-nowrap items-center gap-1 rounded-md px-2.5 py-1 text-[11px] font-semibold ${info.tone}`}><span className="h-1.5 w-1.5 rounded-full bg-current" />{info.label}</span>{assessment.progress?.totalItems > 0 && <span className="mt-1 block text-[11px] text-[#707E79]">{assessment.progress.answered}/{assessment.progress.totalItems} answered</span>}</td><td className="break-words px-3 py-4 text-right sm:px-4">{completed ? <Button as={Link} to={`/assessments/${assessment._id}/result`} size="sm" variant="outline">View result <ArrowRight className="h-3.5 w-3.5" /></Button> : <Button type="button" size="sm" variant="orange" loading={opening} onClick={() => onOpen(assessment)}>{active ? "Resume" : "Start"} <ArrowRight className="h-3.5 w-3.5" /></Button>}</td></tr>;
}
