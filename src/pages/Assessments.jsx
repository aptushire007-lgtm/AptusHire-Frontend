...
const STATUS = {
  scheduled: { label: "Invited", tone: "bg-brand-50 text-primary" },
  not_started: { label: "Assigned", tone: "bg-brand-50 text-primary" },
  in_progress: { label: "In progress", tone: "bg-brand-50 text-primary" },
  completed: { label: "Completed", tone: "bg-brand-50 text-primary" },
  submitted: { label: "Completed", tone: "bg-brand-50 text-primary" },
  expired: { label: "Expired", tone: "bg-red-50 text-red-700" },
  cancelled: { label: "Cancelled", tone: "bg-[#F3F5F3] text-text-muted" },
};

function statusInfo(status) {
  return STATUS[String(status || "").toLowerCase()] || { label: status || "Assigned", tone: "bg-[#F3F5F3] text-text-muted" };
}
  ...
  return (
    <Card className="border-border bg-white dark:border-border dark:bg-white">
      <div className="flex items-start justify-between gap-4">
        <div className="flex min-w-0 items-start gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${completed ? "bg-brand-50 text-primary" : "bg-[#F3F5F3] text-text-muted"}`}><ClipboardList className="h-5 w-5" /></span><div className="min-w-0"><h3 className="truncate text-[15px] font-semibold text-text-strong">{title}</h3><p className="mt-1 truncate text-xs text-text-muted">{assessment.job?.company?.name || "Company"} · {assessment.job?.title || "AptusHire assessment"}</p></div></div>
        <span className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold ${info.tone}`}>{info.label}</span>
      </div>
      <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-xs text-text-muted"><span className="inline-flex items-center gap-1.5"><Clock3 className="h-4 w-4" />Invited {dateTime(date)}</span>{assessment.expiresAt && <span>Deadline {dateTime(assessment.expiresAt)}</span>}{assessment.progress && <span>{assessment.progress.completedSections} of {assessment.progress.totalSections} sections complete</span>}</div>
      {completed && <p className="mt-3 text-xs font-semibold text-text-strong">Overall score: {assessment.overallScore == null ? "Not available" : `${assessment.overallScore}%`}</p>}
      {assessment.progress?.totalItems > 0 && <div className="mt-4"><div className="h-1.5 overflow-hidden rounded-full bg-brand-50"><div className="h-full rounded-full bg-primary" style={{ width: `${Math.min(100, Math.round((assessment.progress.answered / assessment.progress.totalItems) * 100))}%` }} /></div><p className="mt-1 text-[11px] text-text-muted">{assessment.progress.answered} of {assessment.progress.totalItems} answered</p></div>}
      <div className="mt-5 flex items-center justify-between border-t border-border pt-4">
        {completed ? <span className="text-xs text-text-muted">Results are reviewed by the hiring team.</span> : <span className="text-xs text-text-muted">{active ? "Continue where you left off." : "Use your invitation link to begin."}</span>}
...
function AssessmentGroup({ title, items, onOpen, openingId }) {
  ...
  return <section><h2 className="mb-3 text-[16px] font-semibold text-text-strong">{title}</h2><div className="grid gap-4 md:grid-cols-2">{items.map((assessment) => <AssessmentCard key={assessment._id} assessment={assessment} onOpen={onOpen} opening={openingId === assessment._id} />)}</div></section>;
}
...
function AssessmentRow({ assessment, onOpen, opening }) {
  ...
  const active = ["in_progress", "paused"].includes(String(assessment.status || "").toLowerCase());
  const date = completed ? assessment.completedAt : assessment.validFrom || assessment.startedAt || assessment.createdAt;
  return <tr className="border-t border-border text-sm text-text-strong hover:bg-[#FBFCFB]"><td className="break-words px-3 py-4 font-semibold sm:px-4">{assessment.job?.company?.name || "Company"}</td><td className="break-words px-3 py-4 sm:px-4"><p className="break-words font-medium">{assessment.title || assessment.job?.title || "Assessment"}</p><p className="mt-1 break-words text-xs text-text-muted">{assessment.job?.title || "Related job"}</p></td><td className="break-words px-3 py-4 sm:px-4"><span className="rounded-md bg-[#EEF0FF] px-2 py-1 text-[11px] font-semibold text-[#5A62D6]">Skills assessment</span></td><td className="break-words px-3 py-4 text-xs text-text-muted sm:px-4">{dateTime(date)}{assessment.expiresAt && <span className="mt-1 block">Due {dateTime(assessment.expiresAt)}</span>}</td><td className="break-words px-3 py-4 sm:px-4"><span className={`inline-flex whitespace-nowrap items-center gap-1 rounded-md px-2.5 py-1 text-[11px] font-semibold ${info.tone}`}><span className="h-1.5 w-1.5 rounded-full bg-current" />{info.label}</span>{assessment.progress?.totalItems > 0 && <span className="mt-1 block text-[11px] text-text-muted">{assessment.progress.answered}/{assessment.progress.totalItems} answered</span>}</td><td className="break-words px-3 py-4 text-right sm:px-4">{completed ? <Button as={Link} to={`/assessments/${assessment._id}/result`} size="sm" variant="outline">View result <ArrowRight className="h-3.5 w-3.5" /></Button> : <Button type="button" size="sm" variant="orange" loading={opening} onClick={() => onOpen(assessment)}>{active ? "Resume" : "Start"} <ArrowRight className="h-3.5 w-3.5" /></Button>}</td></tr>;
}
