      ...
      {groups.map((group) => (
        <div key={group.label || "public"} className="mb-4 last:mb-0">
          {group.label && !collapsed && (
            <p className="mb-1 px-3 pt-2 text-[10px] font-semibold uppercase tracking-widest text-text-faint">
              {group.label}
            </p>
          )}
              ...
              >
                {({ isActive }) => {
                  ...
                  return (
                    <>
                      <item.icon
                        className={`h-4 w-4 shrink-0 transition-colors ${active ? "text-primary" : "text-text-faint"}`}
                        aria-hidden="true"
                      />
                      <span className={collapsed ? "sr-only" : "truncate"}>{item.label}</span>
                      {item.badgeKey && !collapsed && counts[item.badgeKey] > 0 && (
                        <span className="ml-auto inline-flex min-w-[1.25rem] items-center justify-center rounded-full bg-accent-gold text-[10px] font-bold text-white px-1.5 py-0.5">
  ...
  if (collapsed) {
    return (
      <Link
        ...
        onClick={onNavigate}
        title="AptusHire"
        className="flex h-16 shrink-0 items-center justify-center border-b border-border px-2 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
      >
        <AptusMark size={32} />
      ...
      {!isAuthenticated ? (
        <>
          <Link
            to="/login"
            onClick={onNavigate}
            className={`${baseAction} text-text hover:bg-brand-50`}
          >
            Log In
          </Link>
          <Link
            to="/register"
            onClick={onNavigate}
            className="tap-target inline-flex items-center justify-center rounded-full bg-primary px-5 py-2.5 text-[14px] font-semibold text-white shadow-card transition-colors hover:bg-primary-dark focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-primary/25"
          >
            Register
          </Link>
        </>
        ...
        <>
          ...
          <button
            ...
            className={`${baseAction} text-text-muted hover:bg-brand-50 hover:text-text`}
          >
...
function ShellInner({ children }) {
  ...
  return (
    <div className="candidate-side flex min-h-screen bg-canvas text-text">
      {/* Skip link */}
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:absolute focus:left-4 focus:top-4 focus:z-[60] focus:rounded-card focus:bg-primary focus:px-4 focus:py-2.5 focus:text-sm focus:font-semibold focus:text-white focus:shadow-soft"
      >
      ...
      <aside
        id={SIDEBAR_ID}
        className={`sticky top-0 hidden h-screen shrink-0 flex-col border-r border-border bg-surface transition-[width] duration-200 motion-reduce:transition-none lg:flex ${
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div
            ...
            className="absolute inset-y-0 left-0 flex w-72 max-w-[85vw] flex-col border-r border-border bg-surface shadow-lift"
          >
            <button
              ref={closeRef}
              type="button"
              ...
              className="tap-target absolute right-3 top-4 inline-flex h-9 w-9 items-center justify-center rounded-control text-text-muted transition-colors hover:bg-brand-50 hover:text-text focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"
            >
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Sticky header */}
        <header className="sticky top-0 z-40 flex h-[68px] items-center justify-between gap-3 border-b border-border bg-surface/95 px-5 shadow-card backdrop-blur-md sm:px-8">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <button
              ...
              className="tap-target -ml-1.5 inline-flex items-center justify-center rounded-control p-1.5 text-text-muted transition-colors hover:bg-brand-50 hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/20 lg:hidden"
            >
